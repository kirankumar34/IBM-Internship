            ...
            <div className="flex items-center justify-between">
                <div className="flex items-center space-x-4">
                    ...
                    <div>
                        ...
                        <p className="text-dark-400 text-sm font-medium">Tracking presence and effort across {data.project?.name || 'Project'}</p>
                    </div>
                </div>
            </div>

            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                {(Array.isArray(data.userActivity) ? data.userActivity : []).map((activity) => (
                    <div key={activity.user?.id || Math.random()} className="bg-dark-800/50 border border-dark-700/50 rounded-3xl p-6 hover:border-primary/30 transition-all group">
                        <div className="flex items-center justify-between mb-6">
                            <div className="flex items-center space-x-4">
                                ...
                                <div>
                                    <h4 className="text-white font-bold">{activity.user?.name || 'Unknown'}</h4>
                                    <p className="text-[10px] text-dark-500 font-black uppercase tracking-widest">{(activity.user?.role || 'user').replace('_', ' ')}</p>
                                </div>
                            </div>
                            <div className="text-right">
                                <p className="text-[10px] text-dark-500 font-black uppercase tracking-widest">Total Effort</p>
                                <p className="text-xl font-black text-primary">{(activity.totalHours ?? 0).toFixed(1)}h</p>
                            </div>
                        </div>
                        ...
                        <div className="space-y-4">
                            <div>
                                ...
                                {Array.isArray(activity.logins) && activity.logins.length > 0 ? (
                            ...
                            <div>
                                ...
                                <div className="space-y-1.5 max-h-24 overflow-y-auto no-scrollbar pr-1">
                                    {(Array.isArray(activity.timeLogs) ? activity.timeLogs : []).slice(0, 3).map((log, idx) => (
                                        <div key={idx} className="flex items-center justify-between text-[11px] font-medium">
                                            ...
                                            <span className="text-white font-bold">{log.duration ?? 0}h</span>
                                        </div>
                                    ))}
                                    {(!Array.isArray(activity.timeLogs) || activity.timeLogs.length === 0) && <p className="text-xs text-dark-600 italic">No time logged yet</p>}
                                </div>
                            </div>
                        </div>
                    </div>
